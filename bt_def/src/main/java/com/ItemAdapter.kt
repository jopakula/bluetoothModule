...
class ItemAdapter(private val listener: Listener, val adapterTape: Boolean): ListAdapter<ListItem, ItemAdapter.MyHolder>(Comparator()) {
    private var oldCheckBox: CheckBox? = null

    class MyHolder(view: View, private val adapter: ItemAdapter, private val listener: Listener, val adapterTape: Boolean) : RecyclerView.ViewHolder(view){
        ...
        fun bind(item: ListItem) = with(b){
            checkBox.visibility = if (adapterTape) View.GONE else View.VISIBLE
            item1 = item
            try {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyHolder {
        ...
        return MyHolder(view, this, listener, adapterTape)
    }
